...
As a best practice, store the `RFQ` message in your [Exchanges database](/pfi/anatomy-of-a-pfi#main-server-entrypoint) so that both you and the Wallet app can access it.
...
Building on the example from the [Exchange API Provider section](/pfi/anatomy-of-a-pfi#exchange-api-provider), your code might look like this:
...
Upon receiving an `RFQ`, the tbDEX SDK will automatically <Web5Url to="/verifiable-credentials/verify-vc#verify-verifiable-credentials">verify the submitted credentials</Web5Url> to guarantee their integrity and authenticity as well as ensure that they meet the `requiredClaims` specified in the `Offering`.

## Create a Quote

Before creating a `Quote` in response to the received `RFQ`, you can review the [protocol definition for Quotes](https://github.com/TBD54566975/tree/main/specs/protocol#quote).
