...
After your customer selects an [Offering](/pfi/creating-offerings), the next step is to send a **Request for Quote (RFQ)** to the PFI in order to initiate the transaction.
...
It includes information about the Offering, payment methods, and any required [verifiable credentials (VCs)](/wallet/managing-credentials).
...
* **payin** - How the customer intends to pay the PFI
  ...
  * **paymentDetails** - The exact details of the payment method. As this information is sensitive, the SDK will convert these details into [salted hashes](https://github.com/TBD54566975/tree/main/specs/protocol#privatedata-1)
* **payout** - How the customer wishes to receive the payout from the PFI
  ...
  * **paymentDetails** - The exact details of the payment method. As this information is sensitive, the SDK will convert these details into [salted hashes](https://github.com/TBD54566975/tree/main/specs/protocol#privatedata-1)
* **claims** - The [VCs that satisfy the requirements of the Offer](/wallet/managing-credentials#select-credentials-that-satisfy-requirements). As this information is sensitive, the SDK will convert these details into [salted hashes](https://github.com/TBD54566975/tree/main/specs/protocol#privatedata-1)
...
## Sign the RFQ
Signing the RFQ ensures its authenticity. You can do so with the customer's [Bearer DID](/glossary#bearer-did):
...
The next message written to the exchange will be a [Quote](/wallet/receive-quote) from the PFI.
You'll need to [poll the PFI](/wallet/receive-quote#polling-for-quote) to receive the `Quote` and all other messages within the exchange, unless you provide an `replyTo` callback when sending the RFQ.
...
:::tip No Callback
If a callback is not provided, you'll need to [poll the PFI](/wallet/receive-quote#polling-for-quote) to receive the `Quote` and all other messages within the exchange.
:::
