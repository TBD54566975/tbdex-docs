...
Once the PFI writes a [Quote](/tbdex/wallet/receive-quote) message to the exchange in response to an [RFQ](/tbdex/wallet/send-rfq), you'd present that Quote to your customer and give them the option to place an **Order**.
...
Then sign the Order with the customer's [Portable DID](/glossary#portable-did):
...
If you provided a [callback URI](/tbdex/wallet/send-rfq#send-rfq-to-pfi) when sending the RFQ, the PFI will send the `OrderStatus` messages there.
