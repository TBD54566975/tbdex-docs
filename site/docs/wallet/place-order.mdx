...
Once the PFI writes a [Quote](/wallet/receive-quote) message to the exchange in response to an [RFQ](/wallet/send-rfq), you'd present that Quote to your customer and give them the option to place an **Order**.
...
If you provided a [callback URI](/wallet/send-rfq#send-rfq-to-pfi) when sending the RFQ, the PFI will send the `OrderStatus` messages there.
