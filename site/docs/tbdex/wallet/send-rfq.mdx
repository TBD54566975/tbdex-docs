...
After your customer selects an [Offering](/tbdex/pfi/creating-offerings), the next step is to send a **Request for Quote (RFQ)** to the PFI in order to initiate the transaction.
...
It includes information about the Offering, payment methods, and any required [verifiable credentials (VCs)](/tbdex/wallet/managing-credentials).
...
* **payout** - How the customer wishes to receive the payout from the PFI
  ...
  * **paymentDetails** - The exact details of the payment method. As this information is sensitive, the SDK will convert these details into [salted hashes](https://github.com/TBD54566975/tbdex/tree/main/specs/protocol#privatedata-1)
* **claims** - The [VCs that satisfy the requirements of the Offer](/tbdex/wallet/managing-credentials#select-credentials-that-satisfy-requirements). As this information is sensitive, the SDK will convert these details into [salted hashes](https://github.com/TBD54566975/tbdex/tree/main/specs/protocol#privatedata-1)
...
The next message written to the exchange will be a [Quote](/tbdex/wallet/receive-quote) from the PFI.
You'll need to [poll the PFI](/tbdex/wallet/receive-quote#polling-for-quote) to receive the `Quote` and all other messages within the exchange, unless you provide an `replyTo` callback when sending the RFQ.
...
:::tip No Callback
If a callback is not provided, you'll need to [poll the PFI](/tbdex/wallet/receive-quote#polling-for-quote) to receive the `Quote` and all other messages within the exchange.
:::
