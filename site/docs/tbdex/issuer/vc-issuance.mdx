...
This guide introduces how to issue <Web5Url to="/verifiable-credentials/what-are-vcs">verifiable credentials (VC)</Web5Url>, a process for establishing trust between [Wallets](/tbdex/wallet/overview) and [PFIs](/tbdex/pfi/overview) in a tbDEX network.
...
If you need to create a Credential Issuance app from scratch, check out the [Credential Issuance Server Setup Guide](/tbdex/issuer/vc-serverSetup) for detailed instructions on the dependencies and packages needed to set up your server.
