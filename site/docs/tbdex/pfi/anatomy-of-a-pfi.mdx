...
Every PFI on the tbDEX network is required to handle its end of the responsibilities as part of each [Message Type](/tbdex/message-types). The tbDEX SDK is organized in such a way that there are three key components you'll be required to implement:
...
The [message types](/tbdex/message-types) that are sent from Wallets to PFIs are: `RFQ`, `Order`, and `Close`. 
...
For example, after writing an `RFQ` to the database, you can [create a Quote](/tbdex/pfi/creating-quotes). 
Similarly, after writing an `Order` to your database, you’ll want to ensure you properly [process that Order](/tbdex/pfi/processing-orders).

### Callbacks

When sending an `RFQ` message, the Wallet may also provide [an optional `replyTo` property](/tbdex/wallet/send-rfq#send-rfq-to-pfi) that allows for them to subscribe to subsequent messages. 
