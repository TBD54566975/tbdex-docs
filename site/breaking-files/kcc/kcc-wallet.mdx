...
:::info
For a detailed guide on Known Customer Credentials for **Issuers**, please refer to the [Issuer's Known Customer Credential Guide](/tbdex/issuer/kcc/kcc-issuer)
:::
...
This request contains information about the Issuer along with a [Presentation Definition](https://developer.tbd.website/web5/verifiable-credentials/presentation-definition). Some of the key information included is:
...
:::info
If the User's Verifiable Credentials do not satisfy the Authorization Request's Presentation Definition then only the `id_token` will be sent to the Issuer's `response_uri`. If the credentials **do** satisfy then a [Verifiable Presentation](https://developer.tbd.website/glossary/#verifiable-presentation) (`vp_token`) will also be sent.  
:::
...
To convert the signed VC JWT into a `VerifiableCredential` object you can use the [`parseJwt()` method](/web5/verifiable-credentials/jwt-to-vc#decoding-jwt). Here is what a decoded **Known Customer Credential** would look like:
