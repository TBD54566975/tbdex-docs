...
<LanguageSwitcher languages="JavaScript, Kotlin" /># Issuing a Known Customer
Credential
...
:::info
For a detailed guide on managing Known Customer Credentials as a Wallet application, please refer to the [Known Customer Credential Guide for Wallet Apps](/tbdex/issuer/kcc/kcc-wallet)
:::

## Environment Setup

If you haven't already, please follow the [Credential Issuance Server Setup Guide](/tbdex/issuer/vc-serverSetup) for detailed instructions on the dependencies and packages needed to set up your server.
...
The IDV flow begins when the customer's agent (e.g. Wallet application) sends an HTTP request to the Issuer's [IDV service endpoint](https://hackmd.io/4cW3D9BVQGCEuf1CEg47jA?both#Issuers-Identity) specified in the Issuer's [Decentralized Identifier (DID) Document](/glossary#did-document).
...
The SOIPv2 Authorization request above that you as the issuer send to your customer contains the following key information about you as well as a [Presentation Definition](/web5/verifiable-credentials/presentation-definition).
...
:::note
`vp_token` is a [Verifiable Presentation](/glossary#verifiable-presentation) that contains certified and authenticated claims about the customer. In this guide, we'll cover an example `vp_token`. To learn more refer to [OpenID's specs](https://openid.net/specs/openid-4-verifiable-presentations-1_0.html#section-6.1-2.2).
:::
...
To convert the signed VC JWT into a `VerifiableCredential` object you can use the [`parseJwt()` method](/web5/verifiable-credentials/jwt-to-vc#decoding-jwt). Here is what your decoded **Known Customer Credential** would look like:
